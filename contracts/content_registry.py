...
class ContentRegistry(gl.Contract):
    ...
    user_counter: bigint
    user_reputation: TreeMap[str, bigint]
    ...
    @gl.public.view
    def get_all_articles(self) -> list[bigint]:
        """Get all article IDs - matches ABI: getAllArticles() view returns (uint256[])"""
        result = []
    ...
    @gl.public.view
    def get_article_count(self) -> bigint:
        """Get total article count - matches ABI: getArticleCount() view returns (uint256)"""
        return len(self.articles)

    @gl.public.view
    def get_user_reputation(self, user: str) -> bigint:
        """Get user reputation score - matches ABI: getUserReputation(address) view returns (uint256)"""
        if user in self.user_reputation:
            return self.user_reputation[user]
        return 0
